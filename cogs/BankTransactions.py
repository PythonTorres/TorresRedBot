...
class BankTransactions(commands.Cog):
    ...
    @tasks.loop(seconds = settings.bankTransactions['loopTime'])
    async def refreshTransactionsLoop(self):
        pool = concurrent.futures.ThreadPoolExecutor()
        await self.loop.run_in_executor(pool, self.refreshTransactions)
        pool.shutdown()

    @commands.Cog.listener()
    async def on_ready(self):
        print('[Cog: BankTransactions] Cog is ready')
        self.loop = asyncio.get_running_loop()
        pool = concurrent.futures.ThreadPoolExecutor()
        await self.loop.run_in_executor(pool, self.main)
        pool.shutdown()
        self.refreshTransactionsLoop.start()
    ...
    def getAllTransactions(self, driver):
        ...
        while True:
            try:
                time.sleep(3)
                elementAtTheTop = wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'statement_desc')))
                time.sleep(3)
                elementAtTheBottom = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, 'div[style="height: 74px;"]')))
                time.sleep(3)
                break
    ...
    def cog_unload(self):
        print('[Cog: BankTransactions] Cog unload is started')
        self.refreshTransactionsLoop.cancel()
        self.dataBase.closeConnectionAndCursor()
        print('[Cog: BankTransactions] Database connection is closed')

def setup(bot):
    bot.add_cog(BankTransactions(bot))